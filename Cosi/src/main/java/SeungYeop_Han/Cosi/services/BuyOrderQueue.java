...
public class BuyOrderQueue {
    ...
    private final PriorityBlockingQueue<Order> buyOrderQueue = new PriorityBlockingQueue<>(
            //생성자 매개변수1: initial capacity
            10,

            //생성자 매개변수2: buyOrderQueue의 comparator 객체
            new Comparator<Order>() {
                ...
                public int compare(Order order1, Order order2) {
                    ...
                    //구매 주문 우선순위 큐에서, 주문들은 그 주문가격이 더 비쌀 수록 더 높은 우선순위를 가진다.
    ...
    ///// 생성자 /////
    public BuyOrderQueue(String coinCode) {
    ...
    public Iterator<Order> getIterator() {
        return this.buyOrderQueue.iterator();
    }
    ...
    public void push(Order order) throws RuntimeException {
        ...
        if (!this.coinCode.equals(order.getCoin().getCode())) {
            throw new RuntimeException("매수 주문 큐[" + this.coinCode + "]에 제시된 주문[" + this.coinCode + "]을 삽입할 수 없습니다.");
        }
        //매도 주문이 제시된 경우
        if (!order.getIsBuying()) {
            throw new RuntimeException("매수 주문 큐에 매도 주문을 삽입할 수 없습니다.");
        }
    ...
    /**
     * 매수 주문 큐에서 가장 우선순위가 높은 주문 하나를 조회합니다.
